...
for interval in sample_intervals:
    df = pd.read_hdf(os.path.join(ROOT_DIRECTORY, data_dir, exchange, "all-{}.h5".format(interval)))
    dfs[interval] = df.loc[start_date:]


def objective(arg_dict):
    strat = KalmanFilterStrategy(
        correlation_window_size=arg_dict["cws"], movement_half_life=arg_dict["mhl"]
    )
    # TODO: Argument `fractional_fee` is erroring out.
