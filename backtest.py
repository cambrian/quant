...
def job(sc, input_path, working_dir):
    ...
    def inside_job(strategy, executor, **kwargs):
        ...
        print(executor)
        ex = executor(
            thread_manager, {dummy_exchange: [BTC_USDT, ETH_USDT]}, size=100, min_edge=0.0005
        )
    ...
    param_spaces = {
        "strategy": [Kalman],
        "executor": [Executor],
        "window_size": range(90, 92, 1),
        "movement_half_life": range(90, 92, 1),
        "trend_half_life": range(3000, 3002, 1),
        "cointegration_period": range(60, 62, 1),
        "maxlag": range(120, 122, 1),
    }
